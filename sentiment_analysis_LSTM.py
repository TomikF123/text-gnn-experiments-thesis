# from https://github.com/lukysummer/Movie-Review-Sentiment-Analysis-LSTM-Pytorch/blob/master/sentiment_analysis_LSTM.py
...
### fixed by Ritian-Li ###
encoded_reviews = [
    [vocab_to_int.get(word) for word in review.split()] for review in all_reviews
]
...
assert len(encoded_reviews) == len(
    encoded_labels
), "# of encoded reivews & encoded labels must be the same!"
...
encoded_labels = np.array(
    [label for idx, label in enumerate(encoded_labels) if len(encoded_reviews[idx]) > 0]
)
...
valid_x, valid_y = (
    padded_reviews[:train_cutoff:valid_cutoff],
    encoded_labels[train_cutoff:valid_cutoff],
)
...
class SentimentLSTM(nn.Module):
    ...
    def forward(self, input_words):
        ...
        sigmoid_out = sigmoid_out.view(
            batch_size, -1
        )  # (batch_size, seq_length*n_output)
